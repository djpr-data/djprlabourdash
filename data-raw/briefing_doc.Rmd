...
hours_text <- dplyr::case_when(
  ...
  hours_sign == -1 ~ paste0(" fell by ", hours_change, "%"),
  hours_change == 1 ~ paste0(" rose by ", hours_change, "%")
)
```
-   The total number of hours worked in Victoria `r hours_text`;
-   Victoria's underemployment rate `r summ_dot_point("A85223450L")`;
-   The youth unemployment rate in Victoria `r summ_dot_point("A84424691V")` (smoothed using a 12-month rolling average);
-   The unsmoothed, original youth unemployment rate is `r get_summ("A84424691V", latest_value, summ_data)` per cent, compared to `r get_summ("A84424691V", prev_value, summ_data)` per cent a month earlier.
